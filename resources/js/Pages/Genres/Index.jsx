...
const Index = ({ genres, genre }) => {
    const [tableData, setTableData] = React.useState(genres);
    const isEditing = !!genre?.id;
        ...
        },
        {
            ...
            cellRenderer: ({ data }) => (
                <div className="flex h-full items-center gap-3">
                    <EditIcon route={route('genres.edit', data?.id)} />
                    <DeleteIcon route={route('genres.destroy', data?.id)} />
                </div>
            ),
        },
    ]);
    ...
    const onSubmit = (data) => {
        ...
        if (isEditing) {
            router.put(route('genres.update', genre.id), data, config);
        } else {
            router.post(route('genres.store'), data, config);
        }
    };
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="mx-auto flex max-w-7xl items-center p-4 md:justify-between">
                <h4 className="hidden dark:text-white md:block">Genres</h4>
